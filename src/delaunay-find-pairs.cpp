...
std::vector<std::pair<int, int>> DelaunayFindPairs::find(std::vector<Point>& points)
{
    ...
    DelaunayWithInfo di(boost::make_zip_iterator(boost::make_tuple(points.begin(), indices.begin())),
			boost::make_zip_iterator(boost::make_tuple(points.end(), indices.end())));
    CGAL_assertion( di.number_of_vertices() == points.size() );

    std::vector<std::pair<int, int>> pairs;
    for (DelaunayWithInfo::Finite_edges_iterator eit = di.finite_edges_begin(); eit != di.finite_edges_end(); ++eit)
    {
